...
class ProductViewSetTestCase(APITestCase):
    ...
    def setUp(self):
        self.user = User.objects.create_user(username="test_user1", email="test_user1@gmail.com",
                                             password="test_user_01")
        self.staff_user = User.objects.create_user(username="test_user2", email="test_user2@gmail.com",
                                                   password="test_user_02", is_staff=True)
        ...
        self.product_1 = Product.objects.create(name='Iphone', description="Iphone 17 pro max",
                                                category=self.category_1, price=5000)
        self.product_2 = Product.objects.create(name='Book', description="Xamsa asari ", category=self.category_2,
                                                price=2000)

        Review.objects.create(product=self.product_1, rating=5, user_id=self.user.id)
        Review.objects.create(product=self.product_1, rating=3, user_id=self.staff_user.id)
        Review.objects.create(product=self.product_2, rating=1, user_id=self.staff_user.id)
